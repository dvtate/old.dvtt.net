<?php
	/**todo
	*Add script to check if user is loged in:
	*	+if so:
	*		-logged in as current user page: show edit btn
	*		-logged in as another user: show no buttons
...
<!DOCTYPE html><html>
	<head>
		...
		<meta name="author" content="original copy by DV Tate Testa - This is auomatically generated"/>
		...
		<div class="page">
			<div class="showfakeback">
				<div id="profiletitle">
					<div id="profileimg-wrap"><img id="profileimg" src="<?php try{echo file_get_contents('imgurl.txt');}catch(Exception $e){echo "//dvtate.com/healthit/dr.png";} ?>" alt="profile image" />
					</div><div id="drid"><h1><b><?php echo file_get_contents('name.txt'); ?></b></h1><hr/><h4><?php echo file_get_contents('tag.txt'); ?></h4></div>
				</div>
			</div><div class="fb-like" data-href="<?php echo "//dvtate.com/healthit".$_SERVER['REQUEST_URI']; ?>" data-layout="button_count" data-action="like" data-show-faces="false" data-share="true"></div> <!--FB like-->
<div class="g-follow" data-href="https://plus.google.com/+Tatetesta" data-height='15' data-rel="{relationshipType}"></div><!--g+ follow-->
<div class="g-plusone" data-size="small" data-annotation="inline" data-width="300" data-href="<?php echo "//dvtate.com/healthit".$_SERVER['REQUEST_URI']; ?>"></div><!--g+1-->
