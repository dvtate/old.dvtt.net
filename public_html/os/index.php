<?php
	...
	function verify(){ // checks if data entered is valid
		...
		// put os reccomendations in array to maintain sanity
...
<html>
	<head>
		...
		<script type="text/javascript"><!--//
			...
			function shownext(){
				osnumber++;//unhides and enables the input boxes
				if (osnumber == 2) {
					os2open = true;
			...
			function chkin(){//make left border red if there is missing content(if content is missing && inputbox is active)
				if (!isValidEmail() || document.getElementById("os1").value.trim() == ""
					|| (document.getElementById("os2").value.trim() == "" && os2open == true)
					|| (document.getElementById("os3").value.trim() == "" && os3open == true)
					|| (document.getElementById("os4").value.trim() == "" && os4open == true)
					|| document.getElementById("navn").value.trim() == ""
					|| document.getElementById("email").value.trim()== ""
				) {
 					document.getElementById("nyos").style.borderLeft = "20px solid red";
					document.getElementById("postit").disabled = true;
				} else
					document.getElementById("nyos").style.borderLeft = "20px solid green";
				...
				if (!isValidEmail() && document.forms["osform"]["email"].value != "") {
					...
					document.forms["osform"]["email"].style.removeProperty('border');
		...
		<div class="main">
			...
			<center>
				<img src="//dvtate.com/os/logos/deb.jpg" height="200px" alt="debian logo"
				/><img height="200px" src="//dvtate.com/os/logos/arch.png" alt="arch linux logo"
				/><img src="//dvtate.com/os/logos/ubuntu.png" height="200px" alt="ununtu logo"
				/><img src="//dvtate.com/os/logos/elem.png" height="200px" alt="elementary os logo" />
			</center>
		</div><br/>
		<div class="main" id="nyos">
			<h2><strong>Suggest an OS:</strong></h2><ul>
				<form action="index.php" id="osform" name="postdat" method="post">
					...
					<input type="text" name="navn" id="navn" onkeyup="chkin()"
					 placeholder="ie-Joe" required="true"
					 title="What do you want your name to show up as?"/><?php echo $nameErr; ?><br/>
					<h4 style="display:inline">Email:</h4>
					<input type="email" name="email" id="email" onkeyup="chkin()"
					 placeholder="example@gmail.com" required="true"
					 title="Your email is used to prevent spam on my website. As a token of respect, I won't send you spam."/><?php echo $emailErr; ?><br/>
					<!--<h4 style="display:inline">Website:</h4>
						http://<input type="text" name="email" id="email" onkeyup="chkin()" placeholder="google.com (blank for none)" required="true" title="If you have a website, your name will become a link to it."/><br/><!--determined to be too much code work and no one will use this-->
					<h4 style="display:inline">Suggested Operating Systems:</h4>
					<input type="text" name="os_1" id="os1" onkeyup="chkin()"
					 required="true" placeholder="ie-MS-DOS"
					 title="Pick an Operating System that you recommend I try, and write about"
					/><input type="text" onkeyup="chkin()" name="os_2" id="os2" placeholder="ie-MS-DOS"
					 title="Pick an Operating System that you recommend I try, and write about on this webpage"
					/><input type="text" name="os_3" id="os3" onkeyup="chkin()" placeholder="ie-MS-DOS"
					 title="Pick an Operating System that you recommend I try, and write about on this webpage"/><input type="text" name="os_4" id="os4" placeholder="ie-MS-DOS" onkeyup="chkin()" title="Pick an Operating System that you recommend I try, and write about on this webpage"/><input type="button" id="anotherosbtn" onclick="shownext()" value="Suggest Another (1/4)"/><?php echo $osErr; ?>

					<ul>
						<input type="submit" id="postit" value="Submit" title="Send your message" title=""/>
					</ul>
				</form>
			</ul>
		</div><br/>
